...
size_t available(string_builder sb)
{
...
void sb_copy_string_with_growth(void)
{
	...
	TEST_ASSERT_EQUAL_size_t(1, capacity(sb));

	sb_addc(sb, '1');
	TEST_ASSERT_EQUAL_STRING("1", sb_copy(sb));
	TEST_ASSERT_EQUAL_size_t(2, capacity(sb));

	sb_addc(sb, '2');
	TEST_ASSERT_EQUAL_STRING("12", sb_copy(sb));
	TEST_ASSERT_EQUAL_size_t(4, capacity(sb));
	...
	TEST_ASSERT_EQUAL_size_t(8, capacity(sb));
...
void sb_clear_resets_and_retains_capacity(void)
{
	...
	size_t small_cap = capacity(sb);

	sb_adds(sb, "aspberry");
	TEST_ASSERT_EQUAL_STRING("Raspberry", sb_copy(sb));
	size_t larger_cap = capacity(sb);
